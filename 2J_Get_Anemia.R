...
#3.Load not imputed labs
Final_LAB_df_notimputed <- read.csv(paste0(outdir,"All_LAB_NOTimputed.csv"),stringsAsFactors = F)
...
##########################################################################################
# Compute Anemia using notimputed Hematocrit_D1_LOW and Hemoglobin_D1_LOW
##########################################################################################
Anemia_df <- as.data.frame(matrix(NA, nrow = nrow(Final_LAB_df_notimputed), ncol = 2))
colnames(Anemia_df) <- c("STUDY_PATIENT_ID","Anemia")

for (i in 1:nrow(Final_LAB_df_notimputed)){
  curr_id <- Final_LAB_df_notimputed[i,"STUDY_PATIENT_ID"]
  curr_lowest_Hematocrit <- Final_LAB_df_notimputed[i,"Hematocrit_D1_LOW"]
  curr_lowest_Hemoglobin <- Final_LAB_df_notimputed[i,"Hemoglobin_D1_LOW"]
  ...
  if(is.na(curr_gender) == T | is.na(curr_lowest_Hematocrit) == T | is.na(curr_lowest_Hemoglobin) == T){
    curr_flag <- NA
  }else if (curr_gender == "M" & (curr_lowest_Hematocrit < 39 | curr_lowest_Hemoglobin < 18)){
    curr_flag <- 1
...
write.csv(Anemia_df,paste0(outdir,"All_Anemia_using_NOTImputedLabs.csv"),row.names = F)
