...
get_important_features_and_cooordinates <-function(perf_dir, select_f_dir, num_top_f){
  ...
  Logreg_fs <- top15_importance_df[,"LogReg_Feature"]
  ...
  unique_important_features_df[which(unique_important_features_df$Features %in% Logreg_fs),"Label_Logreg"] <- "Logreg"
  ...
  for (i in 1:nrow(unique_important_features_df)){
    curr_labels <- unique_important_features_df[i,c("Label_Logreg","Label_RF","Label_SVM","Label_XGB","Label_Selected")]
...
plot_topfeatures_func<-function(unique_important_features_df,title_name){
  p <- ggplot(unique_important_features_df, aes(x=X, y=Y,color = Final_Group)) +
    ...
    scale_colour_manual(breaks = c("Logreg, SVM",
                                   "Logreg, XGB",
                                   "Logreg",
                                   "RF, SVM, XGB",
                                   "RF, XGB",
...
plot_topfeatures_func_MAKE<-function(unique_important_features_df,title_name){
  p <- ggplot(unique_important_features_df, aes(x=X, y=Y,color = Final_Group)) +
    ...
    theme(panel.border = element_rect(colour = "black", fill=NA, size=2)) + 
    theme(legend.position="bottom",legend.title = element_blank(),legend.text=element_text(size=20)) +
    scale_colour_manual(breaks = c("Logreg, XGB",
                                   "Logreg",
                                   "RF, XGB",
                                   "SVM",
                                   "Selected",
...
plot_venn_func <- function(set_list){
  p <- venn(set_list, ilab=FALSE,snames = c("LogReg","Selected","RF","XGBoost","SVM") ,
            zcolor = c("darkgreen","darkred","darkblue","darkorange","yellow"),ggplot = T,
            opacity = 0.5,plotsize = 50)
  return(p)
}
...
mortality_plot <- plot_topfeatures_func(unique_important_f_df,"A.Hospital Mortality")
png(paste0(outdir,"Mortality_Important_feature_plot.png"),width = 3000,height = 3000,res = 300)
print(mortality_plot)
dev.off()

#Ven diagram
Log_reg_fs <- unique_important_f_df$Features[which(is.na(unique_important_f_df[,"Label_Logreg"])==F)]
...
png(paste0(outdir,"Mortality_Important_feature_Venn.png"),width = 500,height = 500,res = 120)
...
unique_important_f_df <- get_important_features_and_cooordinates(perf_dir,select_f_dir,15)
MAKE_plot <- plot_topfeatures_func_MAKE(unique_important_f_df,"B.MAKE")
...
scatter_plot <- ggarrange(mortality_plot,MAKE_plot, 
                #labels = c("A", "B"),
                ncol = 1, nrow = 2)
png(paste0(outdir,"Mortality_MAKE_scatter.png"),width = 2000,height = 2000,res = 180)
print(scatter_plot)
dev.off()
