...
for (i in 1:length(analysis_ID)){
  ...
  curr_hosp_start <- ymd_hms(curr_time_df[,"Corrected_HOSP_ADMIT_DATE"])
  curr_hosp_end   <- ymd_hms(curr_time_df[,"Corrected_HOSP_DISCHARGE_DATE"])
  
  curr_icu_start <- ymd_hms(curr_time_df[,"First_ICU_ADMIT_DATE"])
  curr_icu_end   <- ymd_hms(curr_time_df[,"First_ICU_DISCHARGE_DATE"])
...
#Resolved baseline by MRDR
length(no_bl_scr_IDs) #25322
...
Final_SCR_df <- read.csv(paste0(outdir,"All_SCR_df.csv"),stringsAsFactors = F)
