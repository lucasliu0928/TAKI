...
sofa1_func <- function(po2,fio2,onMV_flag){
  if (is.na(po2) == T | is.na(fio2) == T){ #if one value is not available
    ...
    po2_fio2_ratio <- (po2/(fio2/100)) #convert FIO2 to actual percentage
...
sofa4_func <- function(bilirubin_value){
  if (is.na(bilirubin_value) == T){ #if one value is not available
    ...
    if (bilirubin_value < 1.2){
      ...
      highest_score <- 2
    }else if(bilirubin_value  >= 6.0 & bilirubin_value < 12){
      highest_score <- 3
...
apche10_func <- function(hemat_value){
...
apche11_func <- function(wbc_value){
...
apche12_func <- function(gcs_value){
...
All_HT_WT_RESP_FIO2_BMI_df <-read.csv(paste0(outdir,"All_HT_WT_RESP_FIO2_NOTimputed.csv"),stringsAsFactors = F)
...
write.csv(SOFA_With_Feature_df,paste0(outdir,"All_SOFA_With_NotImputedFeature.csv"),row.names = F)
...
for (i in 1:length(analysis_ID)){
  ...
  curr_apche_wbc_low <- apche11_func(curr_wbc_low)
  curr_apche_wbc_high <- apche11_func(curr_wbc_high)
  APACHE_SCORE_df[i,"APACHE_WBC"]<- max(c(curr_apche_wbc_low,curr_apche_wbc_high), na.rm = T)

  
  #12. Apache APACHE_GCS
  curr_gcs <- curr_feature_df[,"GCS"]
  APACHE_SCORE_df[i,"APACHE_GCS"] <- apche12_func(curr_gcs)
  
  #13. Apache APACHE_AGE
  curr_age <- curr_feature_df[,"AGE"]
  APACHE_SCORE_df[i,"APACHE_AGE"] <- apche13_func(curr_age)
...
#Combine feature df and sofa df for check
APACHE_With_Feature_df <- cbind(APACHE_SCORE_df, SOFA_APACHE_Feature_df[,-1])

write.csv(APACHE_With_Feature_df,paste0(outdir,"All_APACHE_With_NotImputedFeature.csv"),row.names = F)
