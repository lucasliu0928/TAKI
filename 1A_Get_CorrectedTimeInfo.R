...
##########################################################################################
#'@updated021122
#### Add 1. D8-D15 start and end time, 
####     2. D8-D15 Start actual hours for each day, and actual days in D4,D5,D6,D7

#### D4 Start  == D3 end + 1 sec, D4 End == the same day of D4 start at 23:59:59
#### D5 Start  == D4 end + 1 sec, D5 End == the same day of D5 start at 23:59:59
#### D6 Start  == D5 end + 1 sec, D6 End == the same day of D6 start at 23:59:59
##########################################################################################
updated_TimeInfo_df <- read.csv(paste0(outdir,"All_Corrected_Timeinfo_ADD_D4toD7.csv"),stringsAsFactors = F)
...
updated_TimeInfo_df$D7_Start <- NA
updated_TimeInfo_df$D7_End <- NA
updated_TimeInfo_df$D8_Start <- NA
updated_TimeInfo_df$D8_End <-NA
...
write.csv(updated_TimeInfo_df,paste0(outdir,"All_Corrected_Timeinfo_ADD_D8toD15.csv"),row.names = F)
