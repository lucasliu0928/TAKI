$(document).ready(function () {
    ...
    function predictMort() {
        ...
        let mhs = checkInput("#id_got_mhs", false, "Mechanical Hemodynamic Support");
        let mechvent = checkInput("#id_got_mechvent", false, "Mechanical Ventillation");
        let sepsis = checkInput("#id_is_septic", false, "Sepsis");
        let vasopress = checkInput("#id_vasopress_exp", false, "Vasoactive Drugs");
...
function checkInput(inputId, checkRange, featureName) {
    ...
    if ($(inputId).val() ==="") {
        ...
        if ((value < minVal) || (value > maxVal)) {
            $(inputId).css('border-color', 'red');
            $(inputId).css('border-width', '2px');
            $("#warntext").append("<br><b>" + featureName + "</b>: Provided value of " + value + " is outside training range of (" + minVal + " - " + maxVal +")");
